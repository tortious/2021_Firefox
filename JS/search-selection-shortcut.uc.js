...
class SearchSelectionShortcut {
    ...
    makePrefs() {
        ...
        SearchSelectionShortcut.prefs.forEach((def, name) => {
            let type;
            // determine the pref type (boolean, number, string). there are a couple more but we
            // won't ever use them.
            switch (typeof def) {
                case "boolean":
                    type = "Bool";
        ...
        // if it's the first time installing v1.6, make a splash menu to reveal the new prefs and
        // offer an affordance to change them.
        if (firstRun) {
    ...
    async setup() {
        ...
        // I tried making this folder in the *actual* Temp directory, but I guess it has a
        // permissions issue or something. when we try that, the hotkey works on system pages, but
        ...
        // this parent file listens for messages from the child file. when it gets a message, it
        // triggers a search or link navigation. the message includes info about the sender, the
        // page's location and CSP, and the selected text or link. if the selected text constitutes
        // a valid link, it will navigate directly to that page. otherwise, it will launch a new
        // browser search using the selected text as a query string. it will normally open the
        // search/link in a new tab. but if you're currently on your new tab page, it assumes you
        // don't want to keep an empty tab around, so it'll open the search/link in the current tab.
        ...
        // this means it'll work on ANY page in ANY browser. it will even work in addon pages loaded
        // in webextension popup panels. for example if you open the uBlock Origin popup from its
        // toolbar button and select some text, the hotkey will search for it in a new tab.
