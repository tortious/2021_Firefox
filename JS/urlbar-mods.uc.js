...
class UrlbarMods {
    static config = {
        // recently the context menu for the search engine one-off buttons in
        // the urlbar results panel has been disabled. but the context menu for
        // the one-off buttons in the searchbar is still enabled. I'm not sure
        // why they did this, and it's a really minor thing, but it's not like
        // right-clicking the buttons does anything else, (at least on windows)
        // so you may want to restore the context menu.
        "restore one-offs context menu": false,

        // when you click & drag the identity box in the urlbar, it lets you
        // drag and drop the URL into text fields, the tab bar, desktop, etc.
        // while dragging it shows a little white box with the URL and favicon
        // as the drag image. this can't be styled with CSS because it's drawn
        // by the canvas 2D API. but we can easily change the function so that
        // it sets the background and text colors equal to some CSS variables.
        // it uses --tooltip-bgcolor, --tooltip-color, and --tooltip-border-color,
        // or if those don't exist, it uses the vanilla variables
        // --arrowpanel-background, --arrowpanel-color, and --arrowpanel-border-color.
        // so if you use my theme duskFox it'll look similar to a tooltip. if
        // you don't use my theme it'll look similar to a popup panel.
        "style identity icon drag box": true,

        // the identity icon is missing tooltips and/or identifying icons for
        // several states. in particular, there is no tooltip on pages with
        // mixed content. on these pages the identity icon generally shows a
        // lock icon with a warning sign on it. so this is the intermediate
        // state between "secure" and "not secure." both of those states have
        // their own special tooltips but the mixed security state does not.
        // (see https://bugzilla.mozilla.org/show_bug.cgi?id=1736354) this is
        // unfortunate because the mixed state is actually a composite of
        // several states. in vanilla firefox, you currently can't tell which
        // one without clicking the identity box to open the popup. hopefully
        // this feature will be added to firefox but for now we can add it
        // ourselves. we add tooltips so the user can distinguish between
        // blocked active content, loaded active content, loaded passive
        // content, weak cipher, or untrustworthy certificate. we also add
        // tooltips to show when the user had HTTPS-only mode enabled but the
        // site failed to load over https. there's also no tooltip to show on
        // chrome UI pages or local files. hovering the identity icon just shows
        // nothing. so we add these as well, so that it should say "This is a
        // secure Nightly page" or "This page is stored on your computer" in a
        // similar way to how it already shows on extension pages. While working
        // on this I noticed that some types of pages are missing a unique
        // class, which means they can't be given a unique icon. these pages are
        // about:neterror and about:blocked. the former shows under many
        // circumstances when a page fails to load; the latter shows when the
        // user has blocked a specific page. if you use duskFox you might have
        // noticed that the theme adds custom icons for certain error pages.
        // previously this was just the HTTPS-only error page and
        // about:certerror (shows when the certificate has a serious problem and
        // firefox won't load the page without user interaction). we couldn't
        // include these other error pages because they were just marked as
        // "unknownIdentity" which is the same as the default class. so I
        // couldn't style them without styling some perfectly valid local or
        // system pages too, such as those with chrome:// URIs. so I'm extending
        // the icon-setting method so it'll give the icon a unique class on
        // these pages: "aboutNetErrorPage" and "aboutBlockedPage" which are
        // pretty self-explanatory. they still keep .unknownIdentity, they just
        // get an extra class. so you can style these yourself if you want but
        // duskFox already styles them just like the other error pages — with
        // the triangular warning sign.
        "add new tooltips and classes for identity icon": true,

        // this sets attributes on each urlbar result according to 1) its
        // corresponding search engine; and 2) its source device, if it's a
        // remote tab. this allows CSS to give them unique icons, or to replace
        // a search engine urlbar result's icon. my theme increases the
        // prominence of the "type icon" in urlbar results. for bookmarks, this
        // is a star. for open tabs, it's a tab icon. for remote tabs, aka
        // synced tabs, it's a sync icon. with this option enabled, however,
        // instead of showing a sync icon it will show a device icon specific to
        // the type of device. so if the tab was synced from a cell phone, the
        // type icon will show a phone. if it was synced from a laptop, it'll
        // show a laptop, etc. the script will add some CSS to set the icons,
        // but it won't change the way type icons are layed out. that's
        // particular to my theme and it's purely a CSS issue, so I don't want
        // the script to get involved in that. my urlbar-results.css file makes
        // the type icon look basically like a 2nd favicon. it goes next to the
        // favicon, rather than displaying on top of it. the script's CSS just
        // changes the icon, so it'll fit with however you have type icons
        // styled. if you don't use my theme but you want type icons to look
        // more prominent, see urlbar-results.css and search "type-icon"
        "show detailed icons in urlbar results": true,

        // normally, when you type something like "firefox install" or "clear
        // cookies" in the urlbar, it suggests an "intervention" or "tip" which
        // acts as a kind of shortcut to various settings and profile operations
        // that some beginners might have a hard time finding. this is a fine
        // feature for people who are new to firefox. but people who use firefox
        // a lot may use the urlbar quickly and could easily accidentally hit
        // enter on one of those results. I decided to add this option to the
        // script because I very nearly wiped my entire profile due to the tip
        // that lets you "Restore default settings and remove old add-ons for
        // optimal performance." From my point of view, these results just waste
        // space while presenting a hazard to the user, which makes them more of
        // a liability than an asset. so they will be removed by this setting.
        "disable urlbar intervention tips": true,

        // by default, urlbar results are not sorted consistently between
        // regular mode and search mode. when you use the urlbar normally, the
        // order of urlbar results is determined by a pref.
        ...
        // urlbar results panel, this pref is no longer used. suggestions are
        // shown at the top of the list no matter what — it's hard-coded into
        // the urlbar muxer's sort function. if you don't change the pref this
        // shouldn't matter, since the suggestions show at the top of the list
        // by default, whether in search mode or not. but if you set the pref to
        // false, (e.g. if you want top or recent site URLs to appear at the top
        // of the list so you don't have to hit Tab so many times to reach them)
        // you'll get URLs at the top of the list in regular mode, and URLs at
        // the bottom of the list in search mode.
        "sort urlbar results consistently": true,

        // when you type nothing but space or tab characters in the urlbar, the
        // first result will have an empty title. consecutive whitespace
        // characters don't add to the displayed node width so it ends up
        // looking basically empty. we can change this by setting it to use
        // non-breaking spaces instead of space characters, and adding an
        // attribute "all-whitespace" to the title element. then your CSS can
        // underline it. this is already done in uc-urlbar-results.css but if
        // you wanna do it yourself: .urlbarView-title[all-whitespace]
    ...
    async extendIdentityIcons() {
        ...
        let [
            ...
            // These strings were intended to be shown as descriptions in the identity popup, not as
            // tooltips on the identity icon. As such, they have trailing periods, but the general
            // convention with tooltips is to omit punctuation. So we need to remove them
            // programmatically in a way that works for any and all translations. Therefore we'll
            // use unicode property escapes with the new property Sentence_Terminal. This should
            // include periods and every equivalent unicode character for sentence terminating punctuation.
            .then((arr) =>
                arr.map((str) =>
        ...
        gIdentityHandler._refreshIdentityIcons = function () {
            ...
            if (this._isSecureInternalUI) {
                ...
                // I'm adding classes here, "aboutNetErrorPage" and "aboutBlockedPage" so that
                // userChrome.css can style them. Since duskFox gives other error pages a warning
                // icon, I want to style these error pages the same icon.
    ...
    styleIdentityIconDragBox() {
        // for a given string in CSS3 custom property syntax, e.g. "var(--tooltip-color)" or
        // "var(--tooltip-color, rgb(255, 255, 255))", convert it to a hex code string e.g.
        // "#FFFFFF"
        ...
        gIdentityHandler.onDragStart = function (event) {
            ...
            try {
                ...
                // Sites might specify invalid data URIs favicons that will result in errors when
                // trying to draw, we can just ignore this case and not paint any favicon.
            }
        ...
        // below is a less extreme, old version. the above adapts to many CSS styled aspects of the
        // urlbar and identity box, to make the drag box look like a more accurate "copy" of the
        // urlbar contents.
        // eval(
    ...
    underlineSpaceResults() {
        gURLBar.view._addTextContentWithHighlights = function (node, text, highlights) {
            ...
            if (!text) return;
            // add an extra attribute to the text node when there are no highlights and the text is
            // short. I was experimenting it but decided not to use it in duskFox. but you can use
            // it in your own CSS. for example...
            // .urlbarView-row[has-url]:not([has-action]) .urlbarView-title[no-highlights] {color: #FF0022}
            // will highlight these titles when the row would otherwise seem kind of dull and empty.
            // the problem with doing that is it kind of obfuscates the meaning otherwise conveyed
            // by the text colors. firefox doesn't really make much use of that by default but
            // duskFox ascribes very specific meanings to the 5 different colors used in urlbar
            // result titles. applying one of them to titles just because they're otherwise empty
            // could mess that up, and it runs the risk of confusing users. so I didn't use it but I
            // left the option open in this script for user customization.
            if (!highlights.length && text.length < 30) node.setAttribute("no-highlights", true);
            // if the string contains nothing but 2 or more whitespace characters, replace them all
            // with a non-breaking space character and set an attribute. CSS can then use
            // .urlbarView-title[all-whitespace] to underline these titles so the otherwise
            // invisible space characters will be visible.
