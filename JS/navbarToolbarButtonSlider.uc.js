...
class NavbarToolbarSlider {
    ...
    /**
     * get the combined width + margin for an element. used for calculating scroll distances.
     * if we used other methods, we'd miss the margins and end up undershooting distances, which is cumulative.
     * intended for use with elements that have box-sizing = border-box.
     * @param {object} el (a DOM node)
    ...
    setMaxWidth(array) {
        ...
        if (this.kids.length) {
            ...
            if (arr)
                arr.slice(0, length).forEach(
                    (el) => (maxWidth += NavbarToolbarSlider.parseWidth(el))
                );
            else {
                widgetList
                    .slice(0, length)
                    .forEach(
                        (w) =>
                            (maxWidth += NavbarToolbarSlider.parseWidth(w.forWindow(window).node))
                    );
            }
        } else
            maxWidth =
                length * NavbarToolbarSlider.parseWidth(document.getElementById("forward-button"));
    ...
    onCustomizeStart() {
        ...
        // temporarily move the slider out of the way. we don't want to delete it since we only
        // want to add listeners and observers once per window. the slider needs to be out of
        // the customization target during customization, or else we get a tiny bug where
        // dragging a widget ahead of the empty slider causes the widget to teleport to the end.
    ...
    onBeforeCommand(aNode) {
        ...
        this.cTarget.appendChild(aNode); // the node must be moved to the customization target, since CustomizableUI expects widgets to be immediate children of a customization target.
    ...
    pickUpOrphans(aNode) {
        ...
        array.forEach((item, i) => {
            /* check that the node which changed is in the customizable widgets list, since the
                ordering logic relies on the widgets list. we use forWindow when selecting nodes
                from the widgets list, since each widget has an instance for every window it's
                visible in. with multiple windows open, array[0] will return an object with a
                property "instances" whose value is an array of objects, each of which has a node
                property referencing the DOM node we actually want. forWindow is just a shortcut to
                get to the object corresponding to the context we're executing in. */
            if (item.id === aNode?.id) {
                /* if the node that changed is the last item in the array, meaning it's
                    *supposed* to be the last in order, then we can't use insertBefore() since
                    there's nothing meant to be after it. we can't only use after() either since it
                    won't work for the first node. so we check for its intended position, and if
                    it's the last item, we use the after() method to put it after the node
                    corresponding to the previous widget. for all the other widgets we just insert
                    their nodes before the node corresponding to the next widget. */
    ...
    // ensure that the order of DOM nodes in the slider container matches the order of widgets
    // in CustomizableUI. rearrange them in whatever way necessary to adhere to the widget
    // order. this is mainly complicated by the fact that the widget array is full of "empty"
    // slots for uninstalled, disabled, or hidden addons, so it can't 1:1 match the DOM. that's
    // also why we use a Boolean filter every time we use the widget array.
    reOrder() {
        let array = this.cuiArray;
        let container = this.isOverflowing ? this.cOverflow : this.inner;
        // for every valid item in the widgets list...
        array.forEach((item, i) => {
            /* if the NODE's next sibling does not match the next WIDGET's node, then we need to
                move the node to where it belongs. basically the DOM order is supposed to match the
                widget array's order. an instance of widget 1 has a property 'node', let's call it
                node 1. same for widget 2, call it node 2. node 1's next sibling should be equal to
                node 2. if node 1's next sibling is actually node 5, then the DOM is out of order
                relative to the array. so we check each widget's node's next sibling, and if it's
                not equal to the node of the next widget in the array, we insert the node before the
                next widget's node. */
            if (
                ...
                /* if nextElementSibling returns null, then it's the last child of the slider.
                    ...
                    but the following expression says to insert the node before the next widget's
                    node. since there is no next widget, we're telling the engine to insert the node
                    before undefined. which always results in inserting the node at the end. so it
                    ends up where it should be anyway. and this is faster than actually checking if
                    it's the last node for every iteration of the loop. */
    ...
    setupScroll() {
        ...
        // container while the popup is open, the popup will move right along with its anchor,
        // the button. this is a problem because some button popups are actually children of the
        // button. meaning mousewheeling with the cursor over the popup would scroll the slider,
        // not the popup. there are other ways to deal with this, but we don't want the slider
        // to scroll at all when the popup is open. because firefox normally blocks scrolling
        // when a menupopup is open. so let's just listen for button nodes having open="true"
        // and set a property on the outer container accordingly. then we can use that prop to
        ...
        for (const [key, val] of Object.entries({
            ...
            clicktoscroll: true,
            orient: "horizontal",
            style: "display: -moz-box; -moz-box-align: center; -moz-box-orient: vertical; scrollbar-width: none; box-sizing: border-box; scroll-behavior: smooth; overflow: hidden; transition: max-width 0.2s ease-out;",
        })) {
        ...
        outer.on_Wheel = function (e) {
            // this is what the mutation observer was for. when a toolbar button in the slider
            // has its popup open, we set outer.open = true. so if outer.open = true we don't
            // want to scroll at all. in other words, if a popup for a button in the slider is
            // open, don't do anything.
            if (this.open) return;
            let doScroll = false;
            let instant;
            let scrollAmount = 0;
            ...
            // if we're scrolling vertically, then use the deltaY as the general delta. if
            // horizontal, then use deltaX instead. you can use this to invert the vertical
            // scrolling direction. just change e.deltaY to -e.deltaY.
            let delta = isVertical ? e.deltaY : e.deltaX;
            // if we're using a trackpad or ball or something that can scroll horizontally and
            // vertically at the same time, we need some extra logic. otherwise it can stutter
            // like crazy. as you see in delta, we want to only use either the deltaY or the
            // deltaX, never both. but if you're scrolling diagonally, that could change very
            // quickly from X to Y to X and so on. so we want to only call scrollBy if the
            // scroll input is consistent in one direction. that's what outer._prevMouseScrolls
            // = [null, null] is for. we want to check that the last 2 scroll events were primarily
            // vertical. if they were, then we'll enable scrolling and set the scroll amount.
        ...
        /**
         * called when focusing a toolbar button with tab or arrow keys. by default, this method
         * calls elem.focus() which scrolls the button into view instantly. we modify the
         * function so that it checks if the button is contained inside a toolbar slider. if it
         * is, then we use a custom scroll function that 1) scrolls smoothly, and 2) scrolls
         * forward/backward such that the element ends up in the center of the scrollbar, unless
         * it's already scrolled to the end/beginning. then it calls a special method that
         * focuses without the native automatic instant scroll behavior.
